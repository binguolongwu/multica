...
-- agent owner has at least one active connection (and the agent has an
-- allowlist). The daemon claim handler merges this on top of agent.mcp_config
-- (overlay wins on name collisions because it carries the live, user-scoped
-- MCP session URL).
--
...
COMMENT ON COLUMN agent_task_queue.runtime_mcp_overlay IS
    'Per-task MCP servers computed at dispatch time, merged on top of agent.mcp_config. Currently used by Composio integration to inject the agent owner''s session URL. Cleared after task completes via trg_clear_runtime_mcp_overlay.';
...
-- transitions live in many different sqlc queries (CompleteAgentTask,
-- FailAgentTask, FailStaleTasks, ExpireStaleQueuedTasks, FailTasksFor
-- OfflineRuntimes, RecoverOrphanedTasksForRuntime, plus several Cancel*
